...
class Tools extends Panel {
	public function left() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			echo '<a target="_blank" href="' . admin_url( 'tools.php?page=debugpress-info&tab=php' ) . '" class="debugpress-button-action">' . esc_html__( "Open PHP Info Panel", "debugpress" ) . '</a>';
			$this->block_footer();
			...
			if ( OPCache::instance()->has_opcache() ) {
				...
				echo '<a target="_blank" href="' . admin_url( 'tools.php?page=debugpress-info&tab=opcache' ) . '" class="debugpress-button-action">' . esc_html__( "Open OPCache Statistics Panel", "debugpress" ) . '</a>';
				$this->block_footer();
			}
			...
			echo '<a target="_blank" href="' . admin_url( 'tools.php?page=debugpress-info&tab=mysql' ) . '" class="debugpress-button-action">' . esc_html__( "Open MySQL Variables Panel", "debugpress" ) . '</a>';
			$this->block_footer();
			...
			echo '<a target="_blank" href="' . admin_url( 'options-general.php?page=debugpress' ) . '" class="debugpress-button-action">' . esc_html__( "Open Settings Panel", "debugpress" ) . '</a>';
			$this->block_footer();
		} else {
	...
	public function middle() {
		...
		if ( current_user_can( 'manage_options' ) ) {
			if ( ! debugpress_is_classicpress() && debugpress_plugin()->wp_version() > 51 ) {
				...
				echo '<a target="_blank" href="' . admin_url( 'site-health.php' ) . '" class="debugpress-button-action">' . esc_html__( "Open Site Health Panel", "debugpress" ) . '</a>';
				$this->block_footer();
			}
			...
			echo '<a target="_blank" href="' . admin_url( 'update-core.php' ) . '" class="debugpress-button-action">' . esc_html__( "Open Updates Panel", "debugpress" ) . '</a>';
			$this->block_footer();
		} else {
			echo '<div class="debugpress-debug-notice-block">';
			$this->title( '<i class="debugpress-icon debugpress-icon-triangle-exclamation"></i> ' . __( "Not Available", "debugpress" ), true, true );
	...
	public function right() {
		...
		if ( ! is_admin() ) {
			...
			echo '<a target="_blank" rel="noopener" href="' . $this->_google_pagespeed_url() . '" class="debugpress-button-action">' . esc_html__( "Run Mobile Test", "debugpress" ) . '</a>';
			echo '<a target="_blank" rel="noopener" href="' . $this->_google_pagespeed_url( 'desktop' ) . '" class="debugpress-button-action">' . esc_html__( "Run Desktop Test", "debugpress" ) . '</a>';
			$this->block_footer();

			$this->block_header();
			$this->sub_title( __( "GTMetrix", "debugpress" ) );
			echo '<p>' . esc_html__( "Open GTMetrix website for the current page.", "debugpress" ) . '</p>';
			echo '<a target="_blank" rel="noopener" href="' . $this->_gtmetrix_url() . '" class="debugpress-button-action">' . esc_html__( "Run the Test", "debugpress" ) . '</a>';
			$this->block_footer();
		} else {
			echo '<div class="debugpress-debug-notice-block">';
